...
export function RecentActivity({ activities }: RecentActivityProps) {
    ...
    const handleActivityClick = (activity: Timeline) => {
        switch (activity.subject_type) {
            case 'Task':
                router.visit(getRoute(`/tasks/${activity.subject_id}`));
                break;
            case 'Meeting':
                router.visit(getRoute(`/meetings/${activity.subject_id}`));
                break;
            // Add more cases for other subject types as needed
            default:
                console.log('Unknown subject type:', activity.subject_type);
        }
    };
